...
- **Onset Lab** — every plant as a dot: **green-up onset** (median day it first breaks leaf) × **leaf-active days** (how many days a year it actually carries leaves, counted week by week). Tap to pin a card; faint plants don't yet have both recorded.
- **Plant Profile** — a downloadable card (PNG + CSV): green-up / flowering / leaf-off days, leaf-active days, the **phenophase calendar** (when each phase happens across years), and low-noise quality checks.
